...
\begin{itemize}
  ...
  \item If your program is short-lived, it may not be necessary (albeit bad practice) to free memory, as the address space of a program gets wiped on return.
\end{itemize}
...
\begin{itemize}
  ...
  \item \texttt{mmap} is another syscall; with the anonymous flag it creates a memory region that is not backed by any file, but is instead associated with swap space. This region can then be treated like a heap and managed as such.
\end{itemize}
\end{document}
